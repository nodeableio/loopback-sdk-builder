...
exports.buildSquema = ctx => {
    ...
    let schema = [
         ...
         */
        {
            ...
            params: {
                ...
                buildNgModuleImports:
                    builders[ctx.framework].buildNgModuleImports,
        ...
        },
        {
            ...
            params: {
                ...
                buildBaseServiceImports:
                    builders[ctx.framework].buildBaseServiceImports
            }
        },
        {
        ...
        },
        {
            ...
            output: `/services/custom/logger.service.${
                ctx.isTyped ? 't' : 'j'
            }s`,
            params: {
                isTyped: ctx.isTyped,
    ...
    Object.keys(ctx.models).forEach(modelName => {
        if (
            ...
            if (
                ...
                schema.push({
                    template:
                        './../templates/shared/services/custom/service.ejs',
                    output: `/services/custom/${modelName}.${
                        ctx.isTyped ? 't' : 'j'
                    }s`,
                    params: {
                        framework: ctx.framework,
                        ...
                        buildRouteParams,
                        buildMethodParams:
                            builders[ctx.framework].buildMethodParams,
                        buildServiceDI: builders[ctx.framework].buildServiceDI,
                        buildServiceImports:
                            builders[ctx.framework].buildServiceImports,
                        normalizeMethodName,
                        buildObservableType,
                        paramIsContext,
                        paramIsFunction
        ...
        Object.keys(model.sharedClass.ctor.relations).forEach(relation => {
            ...
            defaultTypeValue =
                ...
                relationType.indexOf('Array') === -1
                    ...
                    : defaultTypeValue;
            output.push(
                `  ${relation}${isInterface ? '?' : ''}${
                    isTyped ? ':' + relationType + defaultTypeValue + ';' : ';'
                }`
            );
        });
        return output.join('\n');
