...
module.exports = function generate(ctx) {
    ...
    if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
        ...
        directories.push(ctx.outputFolder + '/realtime');
    }
    if (ctx.isNgrx === 'orm') {
    ...
    if (ctx.isNgrx === 'orm') {
        Object.keys(ctx.models).forEach(modelName => {
            for (const rel in ctx.models[modelName].sharedClass.ctor
                .relations) {
                if (
                    ...
                    if (
                        ...
                        throughModels[throughModel] = {
                            from: modelName,
                            to:
                                ctx.models[modelName].sharedClass.ctor
                                    .relations[rel].targetClass
                        };
                    }
                }
            }
    ...
    if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
        schema = schema.concat([
             ...
             */
            {
                template: './shared/realtime/index.ejs',
                output: '/realtime/index.ts',
                params: { models: ctx.models }
            },
            {
                template: './shared/realtime/realtime.module.ejs',
                output: '/realtime/realtime.module.ts',
                params: { models: ctx.models }
            },
    ...
    Object.keys(ctx.models).forEach(modelName => {
        if (
            ...
            if (!ctx.quiet) {
                console.warn(
                    'LoopBack SDK Builder: %s model was ignored',
                    modelName
                );
            }
            ...
            if (!ctx.quiet) {
                console.info(
                    'LoopBack SDK Builder: adding %s model to SDK',
                    modelName
                );
            }
            ...
            schema.push(
                 ...
                 */
                {
                    ...
                    params: {
                        ...
                        modelName: modelName,
                        plural:
                            ctx.models[modelName].sharedClass.ctor.settings
            ...
            if (ctx.isNgrx === 'enabled' || ctx.isNgrx === 'orm') {
                    ...
                    params: {
                        ...
                        plural:
                            ctx.models[modelName].sharedClass.ctor.settings
                                .plural || ejs.filters.pluralize(modelName),
                        moduleName: ctx.moduleName,
                        paramField: ctx.models[modelName].sharedClass.ctor
                            .settings.sdk
                            ? ctx.models[modelName].sharedClass.ctor.settings
                                  .sdk.paramField || 'code'
                            : 'code',
                ...
                schema.push({
                    template: './shared/realtime/service.ejs',
                    output: '/realtime/' + capitalize(modelName) + '.ts',
                    params: {
                        modelName: modelName,
                        paramField: ctx.models[modelName].sharedClass.ctor
                            .settings.sdk
    ...
    function getUserModelName() {
        ...
        Object.keys(ctx.models).forEach(modelName => {
            if (
                ctx.models[modelName].sharedClass.ctor.settings.base ===
                    'User' ||
                ctx.models[modelName].sharedClass.ctor.settings.base ===
                    'OAuthUser'
            ) {
    ...
    function buildObservableType(model, method) {
        ...
        if (
            ...
            (typeof method.returns === 'object' &&
                (String(method.returns.type).toLowerCase() === 'array' ||
                    Array.isArray(method.returns.type)))
        )
            type = `${model.name}[]`;
        if (method.name.match(/(^create$|upsert|^findBy|^findOne$)/g))
            type = model.name;
        return type;
    }
    ...
    function buildOptions(model) {
        return JSON.stringify(
            model.sharedClass.ctor.definition.settings.codeable || {}
        );
    }

    function buildSearchType(model) {
        const search = {};

        Object.keys(model.properties).forEach(property => {
            if (model.properties[property].search) {
                search[property] = model.properties[property].search;
            }
        });

        return JSON.stringify(search);
    }
    function getRouteParam(model) {
        let routeParam =
            Object.prototype.toString.call(model.sharedClass.ctor.getIdName) ==
            '[object Function]'
                ? model.sharedClass.ctor.getIdName()
                : 'id';

        Object.keys(model.properties).forEach(property => {
            if (model.properties[property].routeParam) {
                routeParam = property;
            }
        });
    ...
    function buildServiceImports(model, loadAccessToken, isIo) {
        ...
        let imports = [
            ...
            {
                module: 'HttpClient, HttpResponse',
            ...
            { module: 'Observable, Subject, BehaviorSubject ', from: 'rxjs' },
    ...
    function buildModuleImports(models, isIo, driver) {
        let imports = [
            ...
            { module: 'LoopBackAuth', from: './services/core/auth.service' },
            {
                ...
                from: './services/custom/logger.service'
            },
    ...
    function buildBaseServiceImports(isIo) {
        let imports = [
            ...
            {
                module:
                    'Observable, BehaviorSubject, Subject,  of, ReplaySubject ',
                from: 'rxjs'
            },
            {
                module: 'catchError, shareReplay, takeUntil, map, tap, filter',
                from: 'rxjs/operators'
            }
        ];
    ...
    function buildImports(imports) {
        return imports
            .map(
                item =>
                    `import ${
                        item.from ? `{ ${item.module} }` : `'${item.module}'`
    ...
    function buildMethodParams(model, methodName, params, isIo) {
            ...
            if (model.sharedClass.ctor.relations[relationName].modelThrough) {
                ...
                if (ctx.models[throughName]) {
                    availableClasses.push(
                        capitalize(
                            model.sharedClass.ctor.relations[relationName]
        ...
        if (isIo)
            params = params.filter(
                param => !param.arg.match(/(fk|data|options)/)
            );
    ...
    function paramIsQuery(param) {
        return (
            (typeof param.http === 'undefined' && // Query is default, if http is not defined we treat it as query param
                (param.arg &&
                    !param.arg.match(/(^id$|fk|^file$|container)/))) || // But only if it is not id, fk, file or container
            (typeof param.http !== 'undefined' &&
                typeof param.http.source !== 'undefined' &&
